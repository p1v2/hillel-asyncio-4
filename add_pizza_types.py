...
class Pizza:
	TYPES = {
		'Margherita': {'prep_time': 4, 'bake_time': 4, 'pack_time': 1},
		'Pepperoni': {'prep_time': 3, 'bake_time': 5, 'pack_time': 2},
		'Veggie': {'prep_time': 2, 'bake_time': 4, 'pack_time': 1}
	}
	
	def __init__(self, pizza_id, pizza_type):
...
class PizzaMaker:
	...
	async def make_pizza(self, pizza):
		prep_time = Pizza.TYPES[pizza.type]['prep_time']
	...
	async def put_into_oven(self, pizza, ovens_semaphore):
		...
		bake_time = Pizza.TYPES[pizza.type]['bake_time']
		await self.make_busy(bake_time, pizza)
	...
	async def pack_pizza(self, pizza):
		pack_time = Pizza.TYPES[pizza.type]['pack_time']
		await self.make_busy(pack_time, pizza)
...
async def console_job(pizzas, workers):
	while True:
		...
		for pizza in pizzas:
			...
			print(
				f'Pizza {pizza.pizza_id} ({pizza.type}): {"*" * status_order}{"-" * (len(Pizza.STATUSES) - status_order - 1)}',
				end='')
...
async def main():
	...
	pizzas = [Pizza(i + 1, pizza_types[i % len(pizza_types)]) for i in range(pizza_amount)]
