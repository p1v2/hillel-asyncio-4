	...
	try:
		while True:
			...
			for pizza in pizzas:
				...
				if active_worker:
					specialty_info = f' (Specialist: {active_worker.specialty})' if isinstance(active_worker,
					                                                                           PizzaSpecialist) else ''
					print(f' (Worker {active_worker.number}){specialty_info}', end='')
				print()
			print()
			await async_sleep(1)
...
async def main():
	...
	workers = [PizzaSpecialist(1, 'Margherita'), PizzaSpecialist(2, 'Pepperoni'), PizzaSpecialist(3, 'Veggie'),
	           PizzaMaker(4)]
	
	if workers_amount > 4:
		workers += [PizzaMaker(i + 5) for i in range(workers_amount - 4)]
